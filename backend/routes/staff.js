...
router.get('/students', async (req, res) => {
  try {
    ...
    const filter = { role: { $in: ['student', 'alumni'] } };
    if (dept && dept !== 'all')    filter.department = new RegExp(dept, 'i');
    if (year && year !== 'all')    filter.batch = year;
    if (status && status !== 'all') filter.status = status;

    const students = await User.find(filter)
      .select('name email department batch role status profilePic designation lastLogin createdAt')
      .sort({ createdAt: -1 })
      .limit(200);
...
router.get('/analytics', async (req, res) => {
  try {
    ...
    const activeCount   = await User.countDocuments({ role: { $in: ['student', 'alumni'] }, status: 'Active' });
    const inactiveCount = await User.countDocuments({ role: { $in: ['student', 'alumni'] }, status: 'Pending' });
    ...
    const userGrowth = await User.aggregate([
      { $match: { role: { $in: ['student', 'alumni'] }, createdAt: { $gte: sixMonthsAgo } } },
      {
        $group: {
          _id: { year: { $year: '$createdAt' }, month: { $month: '$createdAt' } },
          count: { $sum: 1 }
    ...
    res.json({
      success: true,
      data: {
        deptStats:    deptStats.map(d => ({ dept: d._id || 'Unknown', count: d.count })),
        activeUsers:  activeCount,
        inactiveUsers: inactiveCount,
        postsPerMonth:  formatMonthly(postsPerMonth),
        userGrowth:     formatMonthly(userGrowth),
        totals: { students: totalStudents, alumni: totalAlumni, posts: totalPosts }
      }
    });
  } catch (err) {
...
router.get('/job-report', async (req, res) => {
  try {
    ...
    const topJobs = await Job.find({ status: 'approved' })
    ...
    console.error('[Staff] job-report error:', err);
